...
export class Pool implements IPool {
  ...
  public static byAsset(asset: Asset, pools: Pool[]): Pool | undefined {
    if (!asset.isRUNE()) {
      return pools.find((pool: Pool) => asset.shallowEq(pool.asset));
    }
  }
  ...
  public static extractPoolMarketCap(
    asset: string,
    oldPools: {pool: any; marketCap: number}[],
  ): number {
    const marketCap = oldPools.find(item => item.pool.detail.asset === asset);
    return marketCap?.marketCap || 0;
  }
