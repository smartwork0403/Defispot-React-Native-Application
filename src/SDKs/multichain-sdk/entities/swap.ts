...
export class Swap implements ISwap {
  ...
  public readonly synthType: SynthType | null = null;
  ...
  constructor({
    ...
    invariant(!inputAsset.isRUNE() || !outputAsset.isRUNE(), 'Invalid pair');
    ...
    const inboundFee =
      ...
      new AssetAmount(
        inputAsset,
        Amount.fromAssetAmount(0, inputAsset.decimal),
      );
    ...
    if (!this.inputAsset.isRUNE() && !this.outputAsset.isRUNE()) {
      ...
      if (this.inputAsset.synth && this.outputAsset.synth) {
  ...
  public static getSingleSwapSlip(
    ...
    // for synth swap slip is 50% of native swap
    if (inputAmount.asset.synth) {
      return new Percent(x.div(x.add(X)).div(2).assetAmount);
    }
